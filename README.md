# qzone-text-exporterv 0.1.0 (qq空间日志纯文本导出器)

把 QQ 空间日志导出成可审计、可检索、可离线处理的纯文本资产，并提供后处理管线（合并、去重、抽样检测、质量分级、索引生成）。

一句话定位：这是把“日志”变成“本地隐私 AI 日记/日志（RAG/知识库）可用输入”的前置条件工具(本身不需要AI,也可以让AI帮你启动).
